...
import { StoreSettings, ThemePreset } from '@/types/database';
...
// ==========================================
// Sub-componentes Refatorados (Etapa 4)
// ==========================================
...
export function PreSignupSettings({ settings, setSettings }: SettingsTabProps) {
...
export function VisitorFormSettings({ settings, setSettings }: SettingsTabProps) {
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-4">
...
export function PostSignupSettings({ settings, setSettings }: SettingsTabProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-4">
        ...
        <div>
          ...
          <select 
            value={settings.post_signup_redirect_mode || 'NONE'} 
            onChange={(e) => {
              ...
              setSettings({ 
                ...
                post_signup_redirect_seconds: secs
              });
            }} 
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold mb-2"
          >
            <option value="NONE">Nenhum redirecionamento automático</option>
            ...
            <option value="ON_CLICK">Somente após clique</option>
          </select>
...
export default function SettingsPage() {
  ...
  // Controle de Abas para a prévia e configurações (PRE, FORM, POST)
    ...
    <div className="flex flex-col xl:flex-row gap-8 max-w-[1400px]">
      ...
      <div className="flex-1 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {activeTab === 'PRE' && (
            <PreSignupSettings settings={settings} setSettings={setSettings} />
          )}

          {activeTab === 'FORM' && (
            <VisitorFormSettings settings={settings} setSettings={setSettings} />
          )}

          {activeTab === 'POST' && (
            <PostSignupSettings settings={settings} setSettings={setSettings} />
          )}
